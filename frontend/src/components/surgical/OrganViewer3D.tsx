import { Suspense, useRef, useState as useReactState, Component, type ReactNode, useLayoutEffect } from "react";
...
export function OrganViewer3D({ organ, zoom, panRef }: OrganViewer3DProps) {
  ...
  // 1. Nuevo estado para atrapar fallos catastróficos de hardware asincrónicos
  const [hardwareError, setHardwareError] = useReactState(false);

  // 2. Escuchamos si Three.js tira un error de Promesa por culpa de la placa de video
  useLayoutEffect(() => {
    const handlePromiseError = (event: PromiseRejectionEvent) => {
      if (event.reason && event.reason.message && event.reason.message.includes("WebGL")) {
        console.warn("Fallo de hardware detectado, cambiando a modo 2D.");
  ...
  // 3. Si no hay soporte webgl nativo o el hardware falló, mostramos las imágenes
